<template>
    <div class="container-fluid mt-0" style="width:60%; margin:auto;">
        <br>
        <h1 class="mx-4 my-2"><i class="fa-solid fa-pen"></i> Modifier l'assignation</h1>
        <br>
...
export default {
    ...
    setup(props) {
        const form = useForm({
            id_enseignant: props.formationEnseignant.id_enseignant,
            id_cours: props.formationEnseignant.id_cours,
            prix_heure: props.formationEnseignant.prix_heure,
            _token: props.csrfToken,
        });

        function submit() {
            form.put(route('formation-enseignants.update', props.formationEnseignant.id), {
                onSuccess: () => {
                    location.reload();
                },
            });
